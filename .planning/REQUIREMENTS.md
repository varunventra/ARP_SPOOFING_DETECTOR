...
- [x] **DET-03**: Tool detects IP-MAC conflicts: when an ARP reply maps an IP to a different MAC than the known table entry, it flags a spoofing event
- [x] **DET-04**: Tool filters out gratuitous ARPs (where `psrc == pdst`) to avoid false positives during normal boot/DHCP cycles
...
- [x] **ALT-02**: Tool displays a live-updating terminal dashboard (rich `Live` / `Table`) showing the current ARP table and recent alerts during monitoring
